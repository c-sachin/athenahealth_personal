...
function executeQuery(){
	var sql_query = 'SELECT DISTINCT DisplayName FROM FlowsheetRowDim';
				...
				if (err1) {
					...
					if(importData.length > 0){
						importData.forEach(function(element,ind) {
							var displayString = element.DisplayName.replace(/\"/g, "");
							displayString = displayString.replace(/\'/g, '');
							
							var sqlQuery1 = 'INSERT INTO vital_signs (`vital_signs_name`, `vital_signs_active`) VALUES ("'+displayString+'", 1)';
